    ...
    <main className="min-h-screen overflow-x-hidden bg-[#F7F4EE] text-[#252922]">
      ...
      <article className="relative mx-auto w-full max-w-6xl px-5 py-8 md:px-8 md:py-12">
        <header className="mx-auto flex max-w-4xl flex-col items-center text-center">
          ...
          <p className="mt-6 text-base text-[#6C716A] md:text-lg">Wiktor Tomasik</p>
        </header>
        ...
        <section className="mx-auto mt-16 w-full max-w-[calc(100vw_-_2.5rem)] md:max-w-3xl">
          ...
          <div className="w-full space-y-7 text-[1.08rem] leading-8 text-[#30352E] md:text-[1.16rem] md:leading-9">
            ...
            <p>Where an LLM works very well as a next-token predictor, it might struggle to predict the consequences of its actions.</p>
            ...
            <p>Another advantage is that current LLMs are shaped by a reward signal to behave safely. But human judgment is not a one-dimensional scalar reward. It contains truthfulness, helpfulness, harmlessness, social context, uncertainty, moral consequences, user intent, and manipulation risk all at once. Compressing this into a single reward score teaches the model to optimize the proxy—making the surface text look safe—without giving it a stable causal model of ownership, evidence, commitment, pressure, and permissible action.</p>
          </div>
        </section>

        <section className="mx-auto mt-24 w-full border-t border-[#1F2420]/10 pt-16">
          <SectionTitle eyebrow="Findings">What I already did</SectionTitle>
          ...
          <div className="mx-auto w-full max-w-[calc(100vw_-_2.5rem)] space-y-7 text-[1.04rem] leading-8 text-[#343932] md:max-w-3xl md:text-[1.1rem] md:leading-9">
            ...
            <p>The important thing I found is not simply that hidden signals exist. The deeper point is that the relation between text, hidden state, and future action is not one-to-one.</p>
          </div>
          ...
          <div className="mx-auto w-full max-w-[calc(100vw_-_2.5rem)] space-y-7 text-[1.04rem] leading-8 text-[#343932] md:max-w-3xl md:text-[1.1rem] md:leading-9">
            ...
            <p>So the best path forward is a semantic world model over action-coupled coordinates. Its job is to learn which hidden variables matter in which context, what happens if we move them, and how to apply the smallest steering needed to avoid the bad transition. This is different from reward-model safety: it is not just asking whether the text looks acceptable, but predicting the consequences of producing that text from that internal state.</p>
          </div>
        </section>
      </article>
    </main>
